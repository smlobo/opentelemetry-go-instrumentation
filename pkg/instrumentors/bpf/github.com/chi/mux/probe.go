...
const instrumentedPkg = "github.com/go-chi/chi/v5"

// Event represents an event in the chi/mux server during an HTTP
// request-response.
type Event struct {
	StartTime   uint64
	EndTime     uint64
...
// Instrumentor is the chi/mux instrumentor.
...
// LibraryName returns the chi/v5 package import path.
func (g *Instrumentor) LibraryName() string {
	return instrumentedPkg
}

// FuncNames returns the function names from "github.com/go-chi/chi/v5" that are
// instrumented.
func (g *Instrumentor) FuncNames() []string {
	return []string{"github.com/go-chi/chi/v5.(*Mux).ServeHTTP"}
}
...
func (g *Instrumentor) registerProbes(ctx *context.InstrumentorContext, funcName string) {
	logger := log.Logger.WithName("chi/v5-instrumentor").WithValues("function", funcName)
	...
	up, err := ctx.Executable.Uprobe("", g.bpfObjects.UprobeChiServeHTTP, &link.UprobeOptions{
		Address: offset,
	})
	if err != nil {
	...
	for _, ret := range retOffsets {
		retProbe, err := ctx.Executable.Uprobe("", g.bpfObjects.UprobeChiServeHTTP_Returns, &link.UprobeOptions{
			Address: ret,
		})
		if err != nil {
...
func (g *Instrumentor) Run(eventsChan chan<- *events.Event) {
	logger := log.Logger.WithName("chi/v5-instrumentor")
	var event Event
	for {
...
func (g *Instrumentor) Close() {
	log.Logger.V(0).Info("closing chi/v5 instrumentor")
